...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            ...
            if (TimeUtil.isBetweenHalfOpen(dateTime.toLocalTime(), startTime, endTime)) {
                boolean excess = caloriesSumPerDate.get(dateTime.toLocalDate()) > caloriesPerDay;

                mealWithExcesses.add(new UserMealWithExcess(dateTime, meal.getDescription(), meal.getCalories(), excess));
            }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesSumPerDate = meals.stream().collect(Collectors.groupingBy(m -> m.getDateTime().toLocalDate(),
        ...
        return meals.stream()
                ...
                .map(m -> new UserMealWithExcess(m.getDateTime(), m.getDescription(), m.getCalories(),
                        caloriesSumPerDate.get(m.getDateTime().toLocalDate()) > caloriesPerDay))
